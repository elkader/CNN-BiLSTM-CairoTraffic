...
data = data.reindex(columns=[data.columns[0],data.columns[1],data.columns[2],data.columns[3],data.columns[4],
                             data.columns[5],data.columns[6],data.columns[7],data.columns[8],'hour','obj'])
data
...
plt.plot(list(dataplot['obj'][0:24*8]),
        label=index, linewidth=3)
plt.title('First week of 2018. New Cairo Station')
...
model3 = Sequential()
model3.add(TimeDistributed(Conv1D(filters=256, kernel_size=2, activation='relu'), input_shape=(4,10,1)))
...
history3=model3.fit(X_train,y_train,epochs=5,batch_size = 32, shuffle=True,validation_data=(X_test,y_test))
...
for i in range(len(pred)):
    if pred[i]<0:
        pred[i] = 0
pred2=pd.cut(flatten(pred), bins, labels = names).astype(str)
ytest2=pd.cut(flatten(y_test), bins, labels = names).astype(str)
...
#We selelct randomly an observation of the entire dataset and we will analyse the 100 successive observations. We plot the actual and the predicted values of these observations.
